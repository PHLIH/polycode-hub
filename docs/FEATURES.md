...
- **网关进程**：`server/src/cli.ts`（入口）+ Hono 应用；运行时**只有一个服务、一个端口**，网关进程自己托管前端构建产物 `web/dist`（`cli.ts:50-53` 探测含 `web/dist` 的目录，`cli.ts:519-520` 挂载 `createAdminUI`）。
- **管理台**：浏览器打开 `http://127.0.0.1:3000/admin/`（`cli.ts:561` 打印该地址），Vue 3 + Element Plus + ECharts（`web/src/views/` 共 6 个页面，见 §7）。
- **存储**：SQLite，`data/admin.db`（4 表：providers / accounts / egresses / admin_meta——建表 `adminapi/store.ts:169-177,225`）与 `data/usage.db`（用量，`cli.ts:131`）分两个库；`data_dir` 缺省 `data`（`config/index.ts:390,402`）。

## 2. 转发面：client 能调什么

挂载位置 `server/src/gateway/proxy.ts:392-405`（`registerRoutes`），由 `cli.ts:515-520` 装到同一 Hono 应用。
...
| POST | `/chat/completions`、`/responses` | 裸路径别名（Base URL 填到端口根的客户端用，`proxy.ts:397-399`） |
| GET | `/v1/models`、`/models` | 模型目录，OpenAI `{"data":[{id}]}` 形态，id 为限定名 `name/modelId`（`proxy.ts:413-427`）；受 `gateway_key` 校验（`proxy.ts:415-418`，失败 401） |
| GET | `/health` | 健康检查，返回 `{ok:true, apps:N}`，N = 启用中的 Provider 数（`proxy.ts:407-411`）；**不鉴权**（注释 `proxy.ts:402-403`） |

- **流式 / 非流式**：都支持。非流式走整体 JSON（`proxy.ts:687-712`）；流式走 SSE（`Content-Type: text/event-stream`，`proxy.ts:879-887`）。
- **client 接入示例**：`export ANTHROPIC_BASE_URL=http://localhost:3000`（README 实测口径）；client 未指定 model 时用 `gateway.default_model` 兜底（`proxy.ts:445`）。
- **网关自身鉴权**：`gateway_key` 为空 = 不校验（回环监听下；监听地址非回环时启动即强制必填，见 `ensureGatewayKey`，§12）；非空则转发面三端点与模型目录要求 `Authorization: Bearer <key>` 并做恒时比较（`proxy.ts:45-52 bearerMatch`，校验点 `proxy.ts:415-418,433-436`）。

## 3. 三协议与 IR 中间表示

- **支持的协议**：`anthropic-messages` / `openai-completions` / `openai-responses`（协议表 `ir/index.ts:5-9`）。
- **转换方式**：统一经 **IR 中间表示**中转——3 个入站解析器 + 3 个出站序列化器，不做 N×N 两两直转（`ir/codec.ts:2`）。三 codec 均双向注册：`codec/anthropicmessages.ts:706-707`、`codec/openaicompletions.ts:1073-1074`、`codec/openairesponses.ts:1136-1137`。
- **IR 词汇表**以 Anthropic Messages 为基准（`ir/types.ts:1-3`）：`IrRequest` 字段见 `ir/types.ts`（model / system / messages / tools / toolChoice / maxTokens / temperature / topP / stopSequences / reasoningEffort / thinkingBudget / stream）；推理强度三协议互转（completions `reasoning_effort` / responses `reasoning.effort` / anthropic `thinking` + `output_config.effort`，未知档位原样透传）；Block 类型 `text | image | tool_use | tool_result | thinking`（`ir/types.ts:8`）；图片源 `base64 | url`（`ir/types.ts:10-16`）；流事件以 Anthropic SSE 事件集为规范形态（`ir/types.ts:190-213`）。
...
- **已知丢弃点**（如实列出，非缺陷隐瞒）：
  - OpenAI Completions 非流式响应序列化丢弃 thinking（`serializeResponse` 内：`codec/openaicompletions.ts:562-575`），`blocksToParts` 只收 text/image（`:419-428`）。
  - OpenAI Responses 丢弃 stopSequences、请求侧 thinking 无形态不处理、未知输出项容忍丢弃。
  - 上游 `reasoning_content`（GLM / DeepSeek 事实标准）→ IR thinking 增量有转换（`codec/openaicompletions.ts:963-968`），出站 thinking → `reasoning_content`（`:624-625`）。
- **图片**：三向支持 text/image；Completions `decodeImageURL / outImageURL`（`decodeImageURL:321` / `outImageURL:778`，base64 组 data URI）；Responses 的 data URI 缺 `;base64,` 会抛错；Completions system-only 场景丢图片（`:310`）。

## 4. 调度：候选排序、换源闸门、冷却

### 候选排序（`router/scheduler.ts`）

1. 过滤：只留 `enabled` 且风险在 `riskMax` 内的 Provider（`scheduler.ts:10-16`，`riskAllowed` 见 `model/index.ts:28-30`）。
...
4. 限定名：`name/modelId` 形态，仅当前缀是已知 Provider 名时才视为限定（`splitModelRef:scheduler.ts:21-30`）；上游只认裸模型名，网关在转发前剥前缀（`proxy.ts:457-459`）。
5. 上下文预检（默认关，`precheck_context: false`）：开启后，用 `len/4` 粗估（`estimateRequestTokens:126-128`）超 `context_window` 的候选被跳过——拒绝 + 换源，**绝不截断**（`pickOrder:84-102`，注释 `:84-85`）。`context_window` 未知（0/缺省）= 放行。
6. 非流式请求跳过 `stream_only` 的源（`pickOrder:94`，如 WorkBuddy 只走流式）。

### 换源闸门（硬约束）

- **首字节前可换源，首字节后绝不换源**（`router/upstream.ts:1-4`，`ir/errors.ts:1-3`）。
- 实现：`Upstream.stream()` 非 2xx / 网络错误抛 `UpstreamError`（可换源）；2xx 返回 body 流，此后不可换源（`upstream.ts:405-410` 的 `stream` 注释与签名）。
- 流式有**首字节闸门**：先等上游吐出第一个真实事件（心跳注释行不算）或超时，再向 client 承诺 200（`proxy.ts:716-770`）；首字节超时默认 15s（`config/index.ts:15`），流中途静默超时默认 60s（`:16`）。闸门已过后上游中途断流，只能补发 error 帧并收尾，不能换源（`proxy.ts:856-863`）。
- 全部候选失败：记一笔 tokens 为 0 的失败账（失败原因进 `errorKind`，供账号维度归因），再把最后一个上游错误映射为对 client 的规范错误（`proxy.ts:580-591`，`mapUpstreamError:proxy.ts:81`）。

### 失败分类与账号冷却

失败分类（`ir/errors.ts:70-78 kindForStatus` + `classifyUpstreamError:upstream.ts:839`）：401/403 → auth（但 403 + RegionError 按 bad_request 处理，不挡协议回退）；402 → quota；429 → rate_limit（但 body 命中配额 hint 则升为 quota）；400/404/413/422 → bad_request；5xx → server。

冷却时长（`proxy.ts:33-38 cooldownFor`）：限流 60s、额度用尽 600s、鉴权 1800s、其他 30s。

账号池（`pool/account.ts`）：

- 同源加权轮询（`pick`）：按 `Account.weight`（缺省/非法按 1）分配流量；无可用抛 `ErrNoAccount`，调度据此换源/报错，不静默等待。
- `markResult`（`pool/account.ts:93-128`）：成功且有惩罚时清零落盘；失败 `fails++`，kind=quota → `exhausted`（**不设到期时间，只认人工“重置”或一次成功的测试**，`pool/account.ts:119-123`），其余 → `cooldown` + 到期时间。
- `hasFor:30-32` 区分“该源无账号 → 走 Provider 级凭据”与“有账号但全冷却 → 跳过该源”。
- 冷却到期自动复位（`eligible:pool/account.ts:167-178`）；`exhausted` 永不自动恢复（`model/index.ts:301-303`）。
- 惩罚落盘：`fails / cooldownUntil` 写回 `admin.db`，重启不失忆；`exhausted` 必须把 status 一起写进去，恢复时再抹回 available（`cli.ts:189-211` 回写规则注释）。
- 健康度（纯派生，不落盘，`model/index.ts:319-331`）：绿=健康、琥珀=冷却中或连败 ≥ 3 次（`FAILS_WARN_AT=3`）、红=disabled / exhausted。
...
- 解析顺序：模型级 `api` → 进程内事实缓存 → Provider 默认 `api`（`resolveProtocol`，`upstream.ts:947-956`）；三者皆无时逐个试候选协议，首个成功者被记住（`upstream.ts:442-455`，`rememberProtocol`）。
...
- 管理台“扫描可用性”：并发打最小真实请求（`maxTokens: 16`，`probe.ts:313`），单模型超时 90s、最多 3 次（`probe.ts:55-60`，仅 rate_limit / quota / server / network 可重试，批量并发 4），成功后把协议写回模型目录持久化。

## 5. 配置：YAML 严格模式与凭据

- **配置文件**：`config/apps.yaml`（本机正式配置，git 忽略）；模板 `config/apps.example.yaml`（214 行，v0.4 口径）。`POLYCODE_CONFIG` 可覆盖路径（`cli.ts:109`）；文件不存在 → 全默认零配置启动（`config/index.ts:50-54`）。
- **gateway 真实字段**（`config/index.ts:18-31`）：`host`（缺省 `127.0.0.1`）、`port`（缺省 3000）、`admin_key`、`gateway_key`（空=不校验；监听地址非回环时启动即强制必填，见 `ensureGatewayKey`）、`default_model`、`risk_max`（low/medium/high，缺省 high=不过滤）、`precheck_context`、`first_byte_timeout_ms`、`stream_idle_timeout_ms`。启动参数 `--port` 可覆盖端口（`cli.ts:113-119`）。
- **Provider 真实字段**（`config/index.ts`）：`name`（旧 `id` 已迁名；只允许小写字母/数字/连字符，名字可改但内部 `providerId` 永不变，`providerValidate:model/index.ts:224-227`）、`display_name`、`access_kind`（official/session-reuse/simulated-login/reverse）、`risk` + `risk_note`（medium/high 必填，`model/index.ts:236-238`）、`stability`、`api`（空=自动探测）、`base_url`（anthropic-messages 停域名根由网关拼 `/v1/messages`，openai 两种停 `/v1`，`buildRequestURL:upstream.ts:1068-1073` 有去重 `/v1` 逻辑）、`credential`、`headers`、`dynamic_headers`（绝对路径命令，不走 shell，缺省超时 8000ms）、`enabled`、`priority`、`stream_only`、`tags`、`models`、`probe_model`、`egress`。
- **严格模式**：未知字段直接抛错拒绝启动（`config/index.ts:309 strictMap`，含 jwt/token 等明文凭据字段亦拒，注释 `:1-2`）；跨实体校验 egress 引用 / 重复 name / provider 字段 / 端口范围（`validate`，`config/index.ts:444-480`）。示例文件本身有测试保证可解析。
- **凭据不落明文**：只存引用 `api_key_env`（环境变量，改值需重启）或 `api_key_file`（`config/credentials/` 下文件，**改内容即热轮换**，每次请求现读，`model/index.ts:36-47`）；无凭据返回 `['', true]` 即公开端点。管理面写凭据文件限定在 `config/credentials/` 下（`adminapi/discover_api.ts:75-86`）。
...
- **存储语义**：YAML 只在**空库时播种一次**（`seedProvidersIfEmpty / seedAccountsIfEmpty`，`adminapi/store.ts:432,479`），之后 `admin.db` 是唯一真相源，界面增删改重启不丢，YAML 后续改动不再生效；配置文件里的 egress 定义在库里缺失时会被补回（`cli.ts:137-138`），Provider 不补回（仅空库播种，删除即终态）。删除 = 软删（`state=deleted`，`DELETE /admin/api/providers/:pid` 成功 204、pid 非法或不存在 404，`adminapi/api.ts:546-556`），物理清行须先软删再 `purge`（未软删先 purge 返回 400，`api.ts:564-578`）。
- **对外监听保护**：`admin_key` 为空合法（零配置启动）；但监听地址非回环（非 `localhost / 127.* / ::1`）且未配 `admin_key` 时**拒绝启动**（`ensureAdminKey`，`cli.ts:73-82`）；`gateway_key` 同款：非回环监听 + 空 `gateway_key` 同样**拒绝启动**（`ensureGatewayKey`，`cli.ts:83-95`）。管理面鉴权：`X-Admin-Key` 或 `Bearer`，恒时比较（`adminapi/api.ts:142-150 safeEqual`，`245-258 withAuth`）。

## 6. 命令行

`npx tsx server/src/cli.ts <子命令>`，或 `node bin/polycode-hub.mjs <子命令>`（bin 仅 5 行 tsx 加载 shim，`bin/polycode-hub.mjs:1-5`）。默认子命令 `serve`（`cli.ts:822`）。

| 子命令 | 作用 |
|---|---|
| `serve`（默认） | 启动网关 + 管理台。参数 `--config <path>`、`--port <n>`（`cli.ts:108-120`） |
| `scan` | 只扫描本机可导入的 harness，不启动服务；`--json` 输出 JSON（`cli.ts:649-661`） |
| `adopt [--id ID] <finding-key>` | 命令行采用某个 harness（如 `workbuddy / opencode-zen`），写入 `admin.db`（`cli.ts:664-696`） |
| `zcode login` | ZCode OAuth 登录：浏览器授权，JWT **只打印一次，不保存**，需自行 export（`cli.ts:702-726`） |
| `zcode sidecar <动作>` | 本地引擎管理：`install / setup / start / stop / status / login / ensure`（缺省 `ensure`，`cli.ts:727-819`）；`install`/`ensure` 支持 `--proxy <url>` 与 `--egress <id>` 指定下载出口，缺省自动复用项目 egress 配置（见 §10 下载代理） |

启动方式：`npm start`（日常）；`npm run dev`（后端 `tsx watch` 热重启）；`./update.sh`（停旧服务 → 前端构建 → 起新服务 → 健康检查打免鉴权 `GET /health`，日志 `data/gateway.log`；停旧服务的命令行 pkill 与端口 kill 都先按进程 cwd 过滤，只杀本仓库的进程，外来进程占端口直接报错不杀）。
...
| 页面 | 作用 |
|---|---|
| Dashboard | 用量仪表盘：365 天热力图 + 归因表 + 胶囊区间；TPS / TTFT 为 null 时显示 —（`usage/store.ts:82-84`，`Dashboard.vue:590-597`） |
| Providers | Provider CRUD + 模型勾选/协议/出口/测试/一键导入；sidecar 一键面板嵌在此页（`Providers.vue:174-182`） |
...
| Login | 管理口令登录（`App.vue:50`） |

管理面 REST（`/admin/api/*`，`adminapi/api.ts:createAdminApi`，错误形状 `{error:{type,message}}`，未接线依赖对应端点 501）：

- egresses：`GET /admin/api/egresses`，`PUT /admin/api/egresses/:id`（body `{kind:http|https, addr}`），`DELETE`（`api.ts:262-287`）。
- providers：`GET` 列表，`POST` 新建（`parse.ts` 收敛解析 + `providerValidate`，冲突 409），`PATCH /:id`（白名单 `name/state/enabled/priority/streamOnly/displayName/riskNote/risk/credential/models/probeModel/egress/stability/accessKind/api/baseUrl/credentialInput/credentialKind/headers`，models 只增不减，headers 整包替换/空对象清空），`DELETE /:id`（软删置 state=deleted，成功 204；pid 非法或不存在 404），`POST /:id/purge`（物理清行，仅限已软删的行，否则 400）。
- accounts：`GET /admin/api/accounts`（DB + 池内冷却/连败合并，过期冷却复位），`POST` 新建（id/providerId 必填，归属 Provider 必须存在；重名 409；新建只许 available/disabled），`PATCH /:id`（白名单 status/displayName/credential/weight/credentialInput/credentialKind），`DELETE /:id`，`POST /:id/recheck`（零上游成本，只清惩罚），`POST /:id/test`（真实请求，可传 `{model}`，成功清冷却归零），`POST /:id/checkin`（WorkBuddy 自动登录：账号页「签到」按钮，只认一键导入/import-account 打标的 importSource=workbuddy 账号；用导入登录态调上游每日签到，一天一次，不自动重试）。
...
- sidecar / projects 以 Hono 子应用注入（`api.ts:1188-1197`），未注入对应端点 501。
...
SQLite 持久化（`usage/store.ts`，schema v5，`user_version` 前向迁移，建表恒最新、缺列 ALTER 且幂等，`:1-16`）。硬规则：`cacheRead / cacheCreation` 独立字段存储，绝不混入 input；accuracy 原样保存，估算值仅展示不可计费；P0 不记金额（`:2-4`）。
...
- **TPS**：总输出 / 总“生成段”耗时（tok/s）。SQL：`avgTps = SUM(守卫内 output) * 1000 / SUM(守卫内 latency_ms - first_token_ms)`，分母已扣 TTFT（decode 段）。守卫：`status='ok' AND stream=1 AND first_token_ms>0 AND latency_ms>first_token_ms`（缺失字段行被排除）；无样本时 `avgTps / avgTtftMs` 为 null，前端显示 —（`usage/store.ts:418-441`，`Dashboard.vue:590-597`）。
- **账号维度**：哪个号被限额一眼认出（`usage/store.ts:106-124 AccountUsage`：requests / errors / errorRate / byKind{quota/rate_limit/auth/…} / tokens / lastErrorAt / models 明细）。
...
- `Sidecar` 句柄（`sidecar.ts:624-642`，选项缺省见 `:160-167`）：二进制缺省 `~/.polycode-hub/bin`，数据 `~/.zcode-proxy`，网关侧凭据引用 `config/credentials/zcode-proxy-key`（`cli.ts:281`），端口缺省 8080，工作目录 `config/zcode-proxy`（`cli.ts:282`）。
- 真实功能：`install [--force]`（下载 GitHub release `TriDefender/zcode-api`，按平台选资产名，已存在跳过，`:653`）；`setupConfig`（写 127.0.0.1 + 24 字节 hex 随机 `sk-local-` key + start-plan，凭据 0600 落盘，`:966`）；`setPort/loadPort`（改/恢复 config.yaml port 行，端口 1024-65535，`:995,1010`）；`start`（分离进程，日志 `logs/sidecar.log`，45s 探活，`:1027,1120`）；`stop`（按进程名 kill，`:1231`，匹配 `killSidecarSpec:356`）；`running`（打 `127.0.0.1:port/health` 带 key，`:1301-1303`（healthOK `:1268-1270`））；`status`（running/installed/stopped/not installed）；`login`（仅交互提示并 spawn 二进制 `auth login zai`，不代持，`:1377`）；`ensureReady`（装→配→起，下载断点续传+重试上限 60 次）；`uninstall`（删二进制+key+config，运行时拒绝）。
- HTTP 仅 `POST /:action=start|stop|reset-guard|login|pause|cancel|setup|uninstall|ensure|port|endpoint` + `GET /` 状态（`sidecar_app.ts:304`、`:362`）；`install / login` 走 CLI 不进 HTTP（`:361,169-171` 明示）；`endpoint` 只改 Provider baseUrl 且仅回环 host（`:492-513`）。
...
- 数据：定义 `config/projects/projects.json`、运行状态 `config/projects/projects-state.json`（`projects/store.ts:93-117`，dir 由 `cli.ts:317` 传入）、日志 `config/projects/logs/<projectID>-<service>.log`（`projects/store.ts:55-65` 路径收敛，启动分隔线 `projects/process.ts:127`，追加写）。日志读取是 1k 条滑动窗口：`readTailLines` 从文件尾按 64KB 块往前扫（`projects_app.ts`），Vite 热更新刷出几十万行也不全量进内存；前端弹窗默认拉 1000 条、贴底跟随、往上滚自动暂停跟随、可选 2s 自动刷新。定义缺失 = 空表不报错；损坏 = 报错（不能静默当空表，否则后续 save 覆盖用户数据，`:67-84`）。
- Service 定义（`projects/store.ts:10-24`）：name / dir / cmd / port（0 = 非网络服务，跳过端口检测）/ portEnv（端口映射方式，缺省 = 端口写死）/ maxRuntimeHours（0/缺省 = 不自动关闭）。
- 启动：端口被占返回 `ConflictError`（API 转 409），带 suggested/remappable（仅 portEnv 已声明的服务可映射）；`startProject` 冲突不阻断其余服务；`stop` 幂等；`restart = stop + start`；巡检每 60s 一次（`cli.ts:580`），超时 kill 留便签 + 死进程清账（`manager.ts:459-460 sweep`）。
...
- EGRESS-SPIKE 方案 A，模型级粒度：Provider 引用 id，未引用 = 直连（`config/index.ts:33-39`）。v1 仅 http/https 代理（undici ProxyAgent 不支持 socks5；Clash 类混合端口用 `http://`，`router/upstream.ts:27-31 egressProxyURI`）。
- 定义：顶层 `egresses: [{id, kind, addr}]`；Provider.egress / Model.egress 引用（模型级覆盖 Provider 级，未声明继承，`proxy.ts:493,620`）。
- 热更新：管理面增删后 `setEgresses` 换表并丢弃旧 dispatcher 缓存（`upstream.ts:363-367`）；请求按 Provider.egress 选 dispatcher，懒建缓存（`:369-377`）。
...
- **上下文预检**：只拒绝 + 换源，绝不截断（`router/scheduler.ts:84-102`）。
...
- **未声明 models 的 Provider 视为透明代理**，接受任意模型（`scheduler.ts:39-43`）。
- **手填模型默认纯文本**：`input` 缺省 `['text']`（`config/index.ts:418`）；支持图片须显式声明，未声明时发出请求前拒绝并点名。
- **失败账**：全部候选失败记一笔 tokens 为 0 的失败账，这是流式失败唯一的落账点（`proxy.ts:580-588`）。
