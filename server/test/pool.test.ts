...
describe('AccountPool（对齐 Go internal/pool/account.go）', () => {
  test('HasFor：有账号（哪怕全冷却）为 true，无账号为 false', () => {
    ...
    expect(pool.hasFor('src')).toBe(true)
    expect(pool.hasFor('other')).toBe(false)
  })
  ...
  test('禁用/耗尽/冷却未到期者被跳过；冷却到期自动复位', () => {
    const pool = new AccountPool([
      acct({ id: 'disabled', status: 'disabled' }),
      acct({ id: 'exhausted', status: 'exhausted' }),
      acct({ id: 'cooling', status: 'cooldown', cooldownUntil: new Date('2026-09-13T13:00:00Z') }),
      acct({ id: 'recovered', status: 'cooldown', cooldownUntil: new Date('2026-09-13T11:00:00Z') }),
    ])
    expect(pool.pick('src', NOW)!.id).toBe('recovered')
    // 复位后池内状态已改写（读回快照验证）
    expect(pool.snapshot().find((a) => a.id === 'recovered')!.status).toBe('available')
  })

  test('无可用账号抛哨兵错误', () => {
    const pool = new AccountPool([acct({ id: 'x', status: 'disabled' })])
    expect(() => pool.pick('src', NOW)).toThrow(/no available account/)
  })
  ...
  test('exhausted 落盘：重启后仍停在耗尽态（不会自己爬起来）', () => {
    ...
    expect(() => boot.pick('src', new Date(NOW.getTime() + 86400_000))).toThrow(/no available account/)
  })
  ...
  test('非 quota 的失败仍走可自动恢复的 cooldown', () => {
    ...
    expect(pool.pick('src', later).id).toBe('a')
  })
  ...
  test('冷却到期复位也落盘（否则 DB 里一直挂着过期冷却）', () => {
    const writes: string[] = []
    const pool = new AccountPool(
      [acct({ id: 'a', status: 'cooldown', cooldownUntil: new Date('2026-09-13T11:00:00Z') })],
      (a) => writes.push(a.id),
    )
    pool.pick('src', NOW)
  ...
  test('重启恢复：DB 里未过期的冷却被照认，过期的丢弃', () => {
    ...
    const stored = [
      acct({ id: 'cool', status: 'cooldown', fails: 3, cooldownUntil: fresh }),
      acct({ id: 'stale', status: 'cooldown', fails: 2, cooldownUntil: new Date(Date.now() - 3600_000) }),
    ]
    const pool = new AccountPool([])
  ...
  test('get：按 id 取账号（副本）；不存在返回 undefined', () => {
    const pool = new AccountPool([acct({ id: 'a' }), acct({ id: 'b', sourceId: 'other' })])
    expect(pool.get('a')!.id).toBe('a')
    expect(pool.get('a')!.sourceId).toBe('src')
    expect(pool.get('nope')).toBeUndefined()
  })

  test('get：冷却到期自动复位（与 pick 同语义）', () => {
    const pool = new AccountPool([
      acct({ id: 'old', status: 'cooldown', cooldownUntil: new Date('2026-09-13T11:00:00Z') }),
    ])
