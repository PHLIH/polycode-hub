...
describe('归因表手动合并（纯函数）', () => {
  ...
  test('mergeRows：zen + zen-auto 合成一行（真实场景）', () => {
    const a = row({ providerName: 'zen', modelId: 'spark', requests: 1694, inputTokens: 100, outputTokens: 10, cacheReadTokens: 90, cacheHitRate: 0.9, totalTokens: 110, avgTps: 100, avgTtftMs: 5000, sampled: 10 })
    const b = row({ providerName: 'zen-auto', modelId: 'spark', requests: 120, inputTokens: 50, outputTokens: 5, cacheReadTokens: 45, cacheHitRate: 0.9, totalTokens: 55, avgTps: 200, avgTtftMs: 3000, sampled: 10 })
    const m = mergeRows([a, b], 'Zen')
    expect(m.providerName).toBe('Zen')
  ...
  test('applyGroups：组内合成一行带明细，未分组行原样', () => {
    const rows = [
      row({ providerName: 'zen', modelId: 'spark', inputTokens: 100 }),
      row({ providerName: 'zen-auto', modelId: 'spark', inputTokens: 50 }),
      row({ providerName: 'wb', modelId: 'hy', inputTokens: 10 }),
    ]
    const out = applyGroups(rows, [{ id: 'g1', name: 'Zen', keys: ['zen/spark', 'zen-auto/spark'] }])
    expect(out).toHaveLength(2)
    expect(out[0]!.providerName).toBe('Zen')
    expect(out[0]!.inputTokens).toBe(150)
    expect(out[0]!._members).toHaveLength(2)
    expect(out[1]!.providerName).toBe('wb')
  })
  ...
  test('applyGroups：组顺序 = 组内首行位置', () => {
    ...
    const out = applyGroups(rows, [{ id: 'g1', name: 'Zen', keys: ['zen/spark', 'zen-auto/spark'] }])
    expect(out[0]!.providerName).toBe('wb')
    expect(out[1]!.providerName).toBe('Zen')
  })

  test('applyGroups：空组列表原样返回；单成员组不合成', () => {
    const rows = [row({ providerName: 'a', modelId: 'm' })]
  ...
  test('applyGroupsReport：正常组不上报', () => {
    const rows = [row({ providerName: 'a', modelId: 'm' }), row({ providerName: 'b', modelId: 'm' })]
