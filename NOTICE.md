...
| OpenCode Zen 免费档（社区整理） | zen 免费档上游 `opencode.ai/zen/v1`；鉴权 `Authorization: Bearer public`（由 `ZEN_KEY` 环境变量提供）；必需请求头 `x-opencode-client` / `x-opencode-project` / `x-opencode-request` / `x-opencode-session`。见 `server/src/discover/index.ts` | 协议事实（端点与请求头），**未复制代码** |
